...
Попаданец в поздней юре: `python examples/run_jurassic_random.py 1000` (seed `150145000`). Харнесс выбирает одно действие по нужде: без боя, без выдуманной зажигалки, без материала не из данных. Сборка — `--build` из объекта с `parts` (стволы, коряга), тег `укрытие`. `--take-resource` сначала наполняет тару с тем же тегом. На этом прогоне: два заслона, 20 снов, все четыре счётчика до `max`, смерть на 690-м от голода (жажда 47) — пустые порции больше не блокируют забор воды. К врагу харнесс не подходит (`RISK_APPROACH_HOSTILE = 0`): голод при охраняемой туше — предел стратегии, не дыра выдачи еды. Огонь 0 — контракт. Ядро юрских имён не знает.
