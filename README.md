...
Попаданец в поздней юре: `python examples/run_jurassic_random.py 1000` (seed `150145000`). Лог: `examples/run_jurassic_log.json`, сводка: `examples/run_jurassic_summary.json`. Перенос «бежал кросс»: шорты, без фляги и зажигалки (остались дома). Вода и сухостой — только с тегами площадки. На этом seed лимит 1000 не достигнут: смерть на 113-м от усталости (`fatigue` 100), после ливня и шторма (`wind_ms` 12). Ночной фронт не успел сработать. Ядро юрских имён не знает.
