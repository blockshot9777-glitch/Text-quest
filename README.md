...
Ожидается ноль ошибок в `audit.py`, **97/97** в `selftest.py`, **21/21** в `sim.py selftest`, код выхода `0`.
...
Другие замыслы и миры: `examples/brief_*.json`, Новгород 1052, Кастор, «Римворлд». Дрейф малого корабля с амнезией: `python examples/run_drift_random.py` (seed `55020714`, 100 ходов, лог `examples/run_drift_log.json`). Счётчики CO₂ и тепла несут `on_complete`: срабатывание меняет `env` отсеков, а не только строку в журнале. Вода и автопилот пока без эффектов — это незаполненный замысел.
