...
Ожидается ноль ошибок в `audit.py`, **67/67** в `selftest.py`, **21/21** в `sim.py selftest`, код выхода `0`.
...
Сценарий каждый ход даёт 4 варианта и выбирает случайный. Лог: `examples/run_1917_log.json`. На seed `19170308` чужак в офисной одежде замерзает на набережной Невы на 18-м ходу: флаг `--fire` одежду сушит, помещение не греет.

Другие замыслы и миры: `examples/brief_*.json`, Новгород 1052, Кастор, «Римворлд».
