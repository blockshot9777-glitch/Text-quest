...
Попаданец в поздней юре: `python examples/run_jurassic_random.py 1000` (seed `150145000`). Лог: `examples/run_jurassic_log.json`, сводка: `examples/run_jurassic_summary.json`. Перенос «бежал кросс»: шорты, без фляги и зажигалки. Вода и сухостой — только с тегами площадки. Смерть на 113-м от `fatigue` 100: пещера с `shelter` есть, сон на ходе 51 уже снимал усталость; дальше харнесс водил к воде. Огонь не зажёгся — нет зажигалки и нет `hearth`, контракт цел. Бой в равный слот не кладётся (первый черновик с схваткой у гнезда из результата убран). Ночной фронт 4/6 — не успел за 49.7 ч, `on_complete` у него есть. Ядро юрских имён не знает.
