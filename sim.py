...
EMBEDDED_RULES = {
 ...
 "cold_model": {
  ...
  "core_drop_span": 70,
  "core_drop_c": 9,
  "note": "T_comf = base - clo_coeff*clo - activity_coeff*активность; прирост = (T_comf - ветрохолод)/divisor. Ядро: base - drop*max(0, need-start)/span. --sheltered или site.shelter: ветер 0. --fire: ambient += fire_bonus_c, топливо списывается по item_use.fuel_per_h."
 },
 "mounts": {
  "рука": 0,
  "тело": 2,
 ...
 "structure_use": {
  "note": "роли конструкции — теги, не имена построек. Часы = объём_л × hours_per_l / (навык/skill_ref × tool_factor если есть tool_tags). Нет блока — --build/--break отказ. Проза в objects[] без parts не ломается.",
  "shelter_tags": [
   "укрытие"
  ],
  "hearth_tags": [
